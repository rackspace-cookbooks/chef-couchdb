...
class Chef
  class Provider
    class CouchdbDatabase < Chef::Provider
      ...
      def action_create
        if exist?
          ...
          resp = couchdb_put(new_resource.database, new_resource.host, options)
      ...
      def action_delete
        if exist?
          resp = couchdb_delete(new_resource.database, new_resource.host, options)
      ...
      def exist?
        resp = couchdb_get('_all_dbs', new_resource.host, options)
        if resp.is_a? Net::HTTPOK
